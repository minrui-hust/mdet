...
class CenterHead(BaseModule):
    def __init__(self,
        ...
        self.shared_conv = nn.Sequential(
                nn.Conv2d(in_channels, shared_conv_channels,kernel_size=3,padding=1,bias=True),
                nn.BatchNorm2d(shared_conv_channels),
                nn.ReLU(inplace=True)
                )

        self.head = SepHead(shared_conv_channels, heads, bn=True, init_bias=init_bias, final_kernel=3)
